...
// Create PID objects
PID motor1PID(&position1, &output1, &setpoint, Kp, Ki, Kd, DIRECT);
PID motor2PID(&position2, &output2, &setpoint, Kp, Ki, Kd, DIRECT);
PID motor3PID(&position3, &output3, &setpoint, Kp, Ki, Kd, DIRECT);
PID motor4PID(&position4, &output4, &setpoint, Kp, Ki, Kd, DIRECT);
...
void loop() {
  ...
  if(isMoving && position1 >= targetDistance)
  {
